...
public class DirectionParticle extends MSObject {

	private float timer = 0;
	private float rotV = 0;
	private float xv, yv;

	public DirectionParticle(int x, int y) {
		super(x, y, Game.MS, Game.MS);
		position.SetZ(4);
		SetSprite(Asset.DIRECTION_PAD_PARTICLE[0]);
	...
	public void Update() {
		...
		try {
			SetSprite(Asset.MOVEMENT_PAD_PARTICLE[(int) (timer * 5)]);
		} catch (Exception e) {
		}
